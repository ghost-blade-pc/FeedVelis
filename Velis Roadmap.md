...
> 定稿基线：2026-09-11。本文是项目目标、架构决策和实施顺序的唯一来源；当前可用功能与运行方式见 [README](README.md)。
>
...
| 协作 | code_copilot | 保留现有阶段协议和证据链，不新建 OpenSpec 平行工作区 |
...
以下为 2026-09-11 代码静态核对，不等同本日运行测试通过；持续更新的现状以 README 为准。
...
| 账户、投稿、审核、资产、Outbox/MQ | 未实现 | I1–I3 |
| Redis 业务、互动、画像、完整 Feed | 未实现 | I5 |
| Eino、OpenSearch、推荐、Agent | 未实现 | I6–I9 |
| 完整 E2E、压测、运维演练 | 未完成 | 各阶段建设，I10 收口 |

不因 I4 的 RSS 能力提前落地而重做它，也不据此宣称 I1–I4 全部完成。当前文章表要求 Source 与外链，不能直接支持站内投稿；RSS 更新直接覆盖正文，接入审核前必须调整。
...
| I1：账户与权限 | 待实施，下一阶段 | 用户、refresh token、注册登录退出、资料、RBAC、JWT、审计上下文与 Vue 登录；依赖 I0 | 正常/过期/撤销/轮换/重复账号/越权测试；管理员初始化方式可复现 |
| I2：投稿发布纵切片 | RSS 模型/阅读可复用 | 统一 Article、版本、图片、稿件状态、审核、详情、latest；同事务 Outbox 表与写入；依赖 I1 | 投稿者创建→编辑→提交→审核员批准→匿名浏览；并发冲突/拒绝/下架/重复请求正确；不依赖 Redis/MQ/搜索/模型 |
...
下一项业务工作为 I1 提案，同时在 I2 前定稿统一文章模型。文档定稿不自动授权业务实现。提案应给出用例、接口、数据、权限、失败路径、测试与迁移方案，再按 code_copilot 阶段执行。

已确定方向不重复征询；以下细节在对应阶段解决，不阻塞本 Roadmap：

| 待定细节 | 最晚定稿阶段 |
| --- | --- |
| 用户名/邮箱登录、开放注册、密码与 Token 策略、管理员初始化 | I1 |
...
- AGENTS/CLAUDE 只维护协作规则与导航；code_copilot 的 project/product context 索引当前事实与目标，不再复制另一份开发蓝图。
- OpenAPI、SQL、测试 fixtures 和局部测试说明继续保留；它们是实现契约，不是额外项目路线。
- 本次文档整理不安装 OpenSearch/Eino、不迁移数据库、不修改业务代码，也不以文档勾选代替执行阶段证据。
