...
| 文章存储 | 按 Source 与条目身份去重、内容哈希、原始正文与清洗正文分离、事务入库 | RSS 直接入库为 published；仅有 published/hidden，无用户投稿、作者发布流程与历史版本 |
...
用户投稿、图片资产、推荐信号、recommend Feed、搜索、Eino AI、对话 Agent 与定时 Agent 均未实现。Redis、RabbitMQ、MinIO 仅有部署配置或代码占位。用户级 RSS 订阅、投稿审核、following/hot Feed 和社交功能已不在目标范围。

抓取器校验 URL、限制重定向并在直连时检查目标地址；环境代理由代理侧解析目标，不能把直连防护等同于代理链路的完整 SSRF 防护。HTML 采用允许列表清洗，远程图片仍由浏览器直接请求。

下一阶段是 Roadmap I2“统一内容供给”：在不依赖 MQ、Redis、搜索或模型的前提下，让管理员维护的 RSS 与用户主动发布的图文稿件进入同一共享内容池。用户发布后直接公开，不建设人工审核流程。
