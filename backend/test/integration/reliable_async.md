...
本文记录 I3 可靠异步底座的可复现验证和最小运维流程。它不包含 AI 模型调用；`async_tasks` 目前只是后续 AI 增强的版本化任务槽位。
...
- 关键指标包括 Outbox pending/oldest age/操作结果、Consumer 结果、任务变化、DLQ 重放、MQ 重连和组件状态。标签不使用 event/task/trace ID、URL、正文或凭据。
...
| MQ 不可达与恢复追赶 | `TestReliableAsyncPipelineAccumulatesWithoutMQAndCatchesUpAfterRecovery` | 文章仍可读、Outbox 积压，恢复后生成 pending task |
